...
@nb.njit
def market_clearing(par,ini,ss,A,A_hh,L_hh,Y, wt, L, Chi, tau, C_hh,K, L_G ,I,clearing_A,clearing_L,clearing_Y, clearing_G, G, L_Y):

    clearing_A[:] = A-A_hh
    L[:] = L_hh
    clearing_L[:] = L_hh - (L_Y + L_G)
    I[:] = K-(1-par.delta)*lag(ini.K,K)
    clearing_G[:] = G + (wt*L_G + Chi) - tau * wt * L_hh # Gov budget constraint
